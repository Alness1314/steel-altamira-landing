...
export function ContactSection({ contact }: ContactSectionProps) {
  ...
  async function handleSubmit(event: { preventDefault: () => void }) {
    ...
    if (!contactEndpoint) {
      ...
      setModalMessage(
        "Configura VITE_BACKEND_URL en el archivo .env para activar el formulario.",
      );
      setIsModalOpen(true);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          getBackendMessage(rawMessage, "No se pudo enviar el mensaje"),
        );
      }
  ...
  return (
    <section
      id="contacto"
      className="section-frame scroll-mt-24 px-3 py-7 sm:px-4 sm:py-8 lg:px-5 lg:py-9"
    >
      <SectionHeading
        title={contact.title}
          ...
          <div className="rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,29,45,0.78),rgba(10,18,29,0.58))] p-5 shadow-[0_18px_50px_rgba(0,0,0,0.25)]">
            <ul className="grid gap-5">
              {contact.details.map((item) => (
                <li
                  key={item.title}
                  className="grid grid-cols-[56px_minmax(0,1fr)] items-start gap-4"
                >
      ...
      {isModalOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 px-4">
          <div className="w-full max-w-md rounded-3xl border border-white/15 bg-[linear-gradient(180deg,rgba(18,29,45,0.95),rgba(10,18,29,0.95))] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
            <h3
              className={`font-display text-2xl font-bold ${isErrorModal ? "text-rose-300" : "text-emerald-300"}`}
            >
